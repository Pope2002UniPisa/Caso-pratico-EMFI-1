...
\title{Caso Pratico -- Economia dei Mercati Finanziari\\[0.3em]
       \large e dell'Informazione \textbf{1}}
\author{Leonardo Pratelli \quad Sara Albotica\\[0.3em]
        \normalsize Università di Pisa\quad|\quad Prof.\ Fulvio Corsi\\
        \normalsize \texttt{fulvio.corsi@unipi.it}\quad|\quad
        Oggetto: \textit{``Caso pratico EMFI 1''}}
...
\[
  ...
  B = \mathbf{1}'\Sigma^{-1}\boldsymbol{\mu}, \quad
  C = \boldsymbol{\mu}'\Sigma^{-1}\boldsymbol{\mu}, \quad
  D = AC - B^2
\]
i cui valori stimati sono riportati nella Tabella~\ref{tab:markowitz}.
La frontiera è la curva $\sigma_p^2(\mu_p) = (A\mu_p^2 - 2B\mu_p + C)/D$.
...
$A$ & $B$ & $C$ & $D$ \\
\midrule
652{,}37 & 6{,}0954 & 0{,}1654 & 70{,}76 \\
\bottomrule
\end{tabular}
\end{table}

Il \textbf{Portafoglio di Minima Varianza Globale (GMV)} ha pesi
$\mathbf{w}^{\text{GMV}} = \Sigma^{-1}\mathbf{1}/A$ e caratteristiche
riportate nella Tabella~\ref{tab:gmv}.
...
Con avversione al rischio $A = 3$, la funzione di utilità
media-varianza
...
\toprule
 & $y^*$ & $\mu_{\text{opt}}$ (\%) & $\sigma_{\text{opt}}$ (\%) & $r_f$ & NVDA & MSFT & JNJ & MRK \\
\midrule
Ottimo & 1{,}669 & 60{,}76 & 44{,}26 & $-66{,}9\%$ & $66{,}1\%$ & $90{,}1\%$ & $-11{,}6\%$ & $42{,}5\%$ \\
\bottomrule
\end{tabular}
\end{table}
...
\[
  ...
  \quad t = 1,\ldots,119,
\]
dove $r_{m,t}$ è il rendimento mensile dell'S\&P~500.
I parametri OLS sono stimati manualmente tramite le formule chiuse:
\[
  \hat\beta_i = \frac{\text{Cov}(r_i, r_m)}{\text{Var}(r_m)}, \qquad
  \hat\alpha_i = \bar r_i - \hat\beta_i\,\bar r_m
\]
con errori standard, $t$-statistica e $p$-value calcolati
usando i residui OLS.
...
\textbf{Commento sugli alpha.}
Solo MSFT e NVDA presentano un $\hat\alpha$ statisticamente
significativo ($p{<}0{,}01$): MSFT con $+14{,}74\%$ annuo e
NVDA con $+37{,}87\%$ annuo. Questi \textit{alpha di Jensen}
indicano un rendimento in eccesso rispetto a quanto previsto
dal CAPM, verosimilmente legato all'impatto dell'intelligenza
artificiale e del cloud computing nel periodo 2015--2024.
Gli altri titoli hanno $\hat\alpha$ non significativi,
coerentemente con l'assenza sistematica di anomalie.
...
\textbf{Media SR bootstrap:} 1{,}507\quad
...
\textbf{Media SR bootstrap:} 1{,}911\quad
